...
int rabbit_bgzf_mul_write_fast(BGZF *fp, BamWriteCompress *bam_write_compress,bam_write_block* &write_block,const void *data, size_t length)
{
    const uint8_t *input = (const uint8_t*)data;
    ssize_t remaining = length;
    while (remaining > 0) {
...
int
rabbit_bam_write_mul_parallel(BGZF *fp, BamWriteCompress *bam_write_compress, bam_write_block *&write_block, bam1_t* b, std::vector<bam_write_block*>& block_vec) {
    const bam1_core_t *c = &b->core;
    uint32_t x[8], block_len = b->l_data - c->l_extranul + 32, y;
    int i, ok = 1;
    if (c->l_qname - c->l_extranul > 255) {
    ...
    if (ok) ok = (rabbit_bgzf_mul_write_fast(fp, bam_write_compress, write_block, b->data, c->l_qname - c->l_extranul) >= 0);
    if (c->n_cigar <= 0xffff) { // no long CIGAR; write normally
        if (ok)
...
#pragma omp parallel for num_threads(THREAD_NUM_PW) schedule(static)
    for(int i = 0; i < vec_size; i++) {
        ...
        rabbit_bam_write_mul_parallel(output->fp.bgzf, bam_write_compress, blocks[tid], b_vec[i],  todo_push_blocks[tid]);
    }
