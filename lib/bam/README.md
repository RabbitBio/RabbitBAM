...
本目录连接 decoded BGZF batch 与 MPE 侧可后处理的 Raw BAM records，同时提供
Composable 过滤和
...
`RunComposableBam1Pipeline` 在 Raw adapter 之后增加一次 MPE 物化。`bam1_t` 和 data
由可复用对象池持有，同样只在当前 `PostProcessBam1Batch()` 回调期间有效；需要跨回调保存时
...
`Bam1Writer` 是该路径的对称写端。输入记录只在 `PostProcessBam1Batch()` 调用期间读取，
